...
| `BINANCE_RECV_WINDOW_MS` (`lib/data/binance-client.ts`), sent as `X-OC-RECV-WINDOW` | 15,000 ms | **Measured.** How far the request timestamp may be from Binance's server time when the request arrives. Binance's default is 5,000 ms and its max 60,000 ms (web3.binance.com/en/dev-docs/authentication). On 2026-09-25 three calls were rejected with 401 / `40103` "Timestamp outside recv_window" after 5.3–9.0 s in transit, with the local clock exact (`docs/devex-log.md`). 15 s covers that with margin. Cost: Binance accepts each signature only once within `2 × recv_window` (30 s here), so a captured request is replayable for up to 15 s rather than 5 s. The header is not part of the signed pre-hash (`timestamp + method + requestPath + body`, same doc, Step 3.1). The app's own request timeout stays 10 s, so a call slower than that is abandoned before the window matters. |
| `BINANCE_REQUEST_TIMEOUT_MS` (`lib/data/binance-client.ts`) | 14,000 ms | **Measured.** How long one Binance call may take before it is abandoned and recorded as a `TimeoutError`. Was 10,000 ms. On 2026-09-25 successful calls took up to 9,919 ms (`docs/devex-log.md`), at the old limit's edge. 14 s stays just under `BINANCE_RECV_WINDOW_MS` (15 s): a request arriving later than that would be rejected with `40103` anyway. It does not help with outages (12:29–12:35 UTC that day, no TLS handshake for 19–25 s). The scheduler's quote and market-status calls run concurrently, so a slow tick costs about one timeout, not two; a tick that outlasts the 30 s interval causes the next one to be skipped and recorded (`scheduler` ledger entry, `tick_skipped`). |
